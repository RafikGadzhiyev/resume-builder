...
export const checkPassword = (password: string, minLength: number, constrains: IPasswordConstraints) => {
	...
	return !(password.replace(/\D/gi, "").length === 0 && constrains.atLeastOneDigit);
};
export const checkPasswordSync = (password: string, repeatedPassword: string) => password === repeatedPassword;
export const isInputEmpty = (input: HTMLInputElement) => input.value.length === 0;
export const InputValidation = (input: HTMLInputElement, isRequired: boolean) => {
